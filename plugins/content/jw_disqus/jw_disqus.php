	...
	function renderDisqus(&$row, &$params, $page)
	{
		...
		if (version_compare(JVERSION, '1.6.0', 'ge'))
		{
			...
			if (in_array($row->access, $levels))
			{
				if ($view == 'article')
				{
					$itemURL = isset($row->readmore_link)?$row->readmore_link:'';
				}
				else
				{
					$itemURL = JRoute::_(ContentHelperRoute::getArticleRoute($row->alias, $row->catid));
				}
			}
		}
		else
		{
			$itemURL = JRoute::_(ContentHelperRoute::getArticleRoute($row->alias, $row->catslug, $row->sectionid));
		}
