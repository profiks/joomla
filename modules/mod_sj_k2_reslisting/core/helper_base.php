...
JLoader::register('ImageHelper', dirname(__FILE__).DS.'helper_image.php');
require_once (JPATH_SITE.DS.'components'.DS.'com_k2'.DS.'helpers'.DS.'route.php');
if (!class_exists('K2BaseHelperResponsiveListing')){
	/**
	 * BaseHelper for com_k2 only.
	 */
	abstract class K2BaseHelperResponsiveListing{
		/**
		...
		*/
		...
		public static function getCategories(&$params){
			...
			if ($params->get('catfilter')){
				...
				if ($categories){
					$list = self::getCategoriesFull($categories, $params);
				}
				
			} else {
				$list = self::getCategoriesChild(0 , 0, $params);
			}
		...
		public static function getTotal($ids,$params){
			...
			if(!empty($ids)){
				$db = JFactory::getDbo();
				$query = "SELECT count(id) as countitems  FROM #__k2_items AS count_table WHERE count_table.catid IN (" . implode(',', $ids)  . ") AND count_table.published = 1";
				$db->setQuery($query);
		...
		public  static function getCategoriesFull($categories, $params){
			$mainframe = JFactory::getApplication();
			$db = JFactory::getDBO();
			$user = JFactory::getUser();
			//$aid = (int)$user->get('aid');
			...
			$cat_order_by = self::_catOrders($params);
			$query .= "	ORDER BY {$cat_order_by}";
		...
		protected static function _catOrders($params){
			...
			if ( isset($source_order_by) ){
				switch ($source_order_by){
					default:
					case 'name':
						$cat_order_by = "name ASC";
						break;
					case 'ordering':
						$cat_order_by = "ordering ASC";
						break;
				}
			}
			return $cat_order_by;
		...
		public static function getCategoriesChild($category=0,$maxlevel=0, $params)
		{
			...
			if (count($list)>0){
				$list = self::getCategoriesFull($list, $params);
			}
			return $list;
		}
