		...
		<div class="userItemView<?php if(!$item->published || ($item->publish_up != $this->nullDate && $item->publish_up > $this->now) || ($item->publish_down != $this->nullDate && $item->publish_down < $this->now)) echo ' userItemViewUnpublished'; ?><?php echo ($item->featured) ? ' userItemIsFeatured' : ''; ?>">
		  ...
		  <div class="userItemBody">
			  ...
			  <?php if($this->params->get('userItemImage')) :
					...
					(!empty($item->imageGeneric))? $imgsrc = $item->imageGeneric : $imgsrc = 'http://placehold.it/250x200' ;
			  ?>
			  <!-- Item Image -->
			  <div class="userItemImageBlock">
				  <span class="userItemImage">
				    <a href="<?php echo $item->link; ?>" title="<?php if(!empty($item->image_caption)) echo K2HelperUtilities::cleanHtml($item->image_caption); else echo K2HelperUtilities::cleanHtml($item->title); ?>">
				    	<img src="<?php echo $imgsrc; ?>" alt="<?php if(!empty($item->image_caption)) echo K2HelperUtilities::cleanHtml($item->image_caption); else echo K2HelperUtilities::cleanHtml($item->title); ?>" style="width:<?php echo $this->params->get('itemImageGeneric'); ?>px; height:auto;" />
				    </a>
				  </span>
				  <div class="clr"></div>
			  </div>
			...
			<span class="userItemDateCreated">
				<i class="icon-time"></i><?php echo JHTML::_('date', $item->created , JText::_('d M Y')); ?>
			</span>
			<?php endif; ?>
			...
			<div class="userItemCommentsLink">
				...
				<?php else: ?>
					...
					<a href="<?php echo $item->link; ?>#itemCommentsAnchor">
						<?php echo JText::_('K2_COMMENTS').": "; echo $item->numOfComments; ?> 
						 <?php //echo ($item->numOfComments>1) ? JText::_('K2_COMMENTS').": " : JText::_('K2_COMMENT').": "; ?><?php //echo $item->numOfComments; ?>
					</a>
				<?php endif; ?>			
