...
defined('_JEXEC') or die;
  ...
  if(!empty($list)){
    JHtml::stylesheet('templates/'.JFactory::getApplication()->getTemplate().'/html/'.$module->module.'/assets/css/style.css');
	...
	<!--[if gt IE 9]><!--><div id="<?php echo $tag_id;?>" class="sj-extraslider <?php if( $options->effect == 'slide' ){ echo $options->effect;}?>  <?php echo $class_respl; ?>"   data-interval="<?php echo $interval; ?>" data-pause="<?php echo $params->get('pause_hover'); ?>"><!--<![endif]-->
        ...
        <?php }?>	
	<div class="extraslider-content <?php echo $options->theme;?>">
    	<div class="extraslider-control  <?php if( $options->button_page == 'under' ){echo 'button-type2';}?>">
		    ...
		    <?php }?>
			<a class="button-extra button-prev" href="<?php echo '#'.$tag_id;?>" data-jslide="prev"></a>
		    <a class="button-extra button-next" href="<?php echo '#'.$tag_id;?>" data-jslide="next"></a>
	    </div>
				...
				<?php }?>
					<div class="item-wrap <?php echo $options->theme;?>">
						<div class="item-wrap-inner">
							<?php if( $options->theme == 'style1'){?>
								<div class="item-image">
									
									<?php 	$extra_video = json_decode($item->extra_fields); echo $extra_video[0]->value;
											//Check placeholder path is exist or not exist?
											foreach($extra_video as $item_video){
												
												$extra_video_flace = isset($item_video-> value) ? $item_video-> value : '';
												if ( $extra_video_flace == '' && $params->get('imgcfg_placeholder')==1){
													$extra_video_flace = $params->get('imgcfg_placeholder_path');
													echo "<img src='".JURI::base().$extra_video_flace."'/>";
												}
											}
							...
							?>
							<div class="item-image">
								<?php if( $options->theme == 'style2'){
									$extra_links = json_decode($item->extra_fields); 
									$extra_link = $extra_links[0]->value;
									?>									
										<a href="<?php echo $extra_link[1] ?>" target="<?php echo $extra_link[2] ?>"><?php echo K2ExtrasliderHelper::imageTag($img);?></a>
									<?php } else{ ?>
										<a href="<?php echo $item->link;?>" title="<?php echo $item->title ?>" <?php echo K2ExtrasliderHelper::parseTarget($params->get('item_link_target')); ?>><?php echo K2ExtrasliderHelper::imageTag($img);?></a>
									<?php } ?>
							</div>
							<?php } }?>
							...
							<?php if( $options->item_title_display == 1 ){?>
								...
								<?php if( $options->theme == 'style4'){
									$extra_team = 	json_decode($item->extra_fields); 
								?>
									<div class="item-regency">
										<?php echo '- '.$extra_team[0]->value.' -';?>
									</div>
								<?php } ?>
							<?php }?>
							<?php if( ($options->item_desc_display == 1 && !empty($item->displayIntrotext)) || ($item->tags != '') || $options->item_readmore_display == 1 ){?>
								<div class="item-content">
									...
									<?php }?>
									<?php if( $options->theme == 'style4'){?>
										<div class="footer-social item-social">
											<?php if( $extra_team[1]->value != "" ){ ?>
											<div class="yt-socialbt">
												<a data-placement="top" target="_blank" class="sb facebook default " title="" href="<?php echo $extra_team[1]->value; ?>"><i class="fa fa-facebook"></i></a>
											</div> 
											<?php } if( $extra_team[2]->value != "" ){ ?>
											<div class="yt-socialbt">
												<a data-placement="top" target="_blank" class="sb twitter default " title="" href="<?php echo $extra_team[2]->value; ?>"><i class="fa fa-twitter"></i></a>
											</div> 
											<?php } if( $extra_team[3]->value != "" ){ ?>
											<div class="yt-socialbt">
												<a data-placement="top" target="_blank" class="sb skype default " title="" href="<?php echo $extra_team[3]->value; ?>"><i class="fa fa-skype"></i></a>
											</div> 
											<?php } if( $extra_team[4]->value != "" ){ ?>
											<div class="yt-socialbt">
												<a data-placement="top" target="_blank" class="sb google-plus default " title="" href="<?php echo $extra_team[4]->value; ?>"><i class="fa fa-google-plus"></i></a>
											</div>
											<?php }?>
										</div>
									<?php }?>
